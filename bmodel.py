...
from torch.utils.data import DataLoader, WeightedRandomSampler
 ...
 #watch out when playing with the number of attention heads. the number of heads multiplied
 #by the number of output channels must add up to the input size
...
class BNET(torch.nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.fnn = torch.nn.Sequential(   #wrapping in sequential because shap has trouble determining model dimensions otherwise
            torch.nn.Linear(in_channels, 8*out_channels),
            torch.nn.LeakyReLU(),  #USING LEAKY RELU INSTEAD OF RELU!!
...
if __name__ == '__main__':
    ...
    HRES_OUT       = 'molgl2htest48-3400v2.txt'
    TRAINING_OUT   = 'molgl2htrainres48-3400v2.csv'
    TEST_REACTS    = 'molgl2htestreacts48-3400v2.txt'
    INPLAYER       = 'molgl2hinlayer48-3400v2.pt'
    OUTLAYER       = 'molgl2houtlayer48-3400v2.pt'
    ONEHOT         = 'onehoth3400v2.pkl'
    ...
    #scheduler3 = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', patience=5, threshold=1e-2)
            ...
            for idx, data in enumerate(trainingloader):
              ...
              # torch.nn.utils.clip_grad_norm_(net.parameters(), 1)
                ...
                if ((idx+1) % accum_iter == 0) or (idx+1 == len(trainingloader)):
                    ...
                    running_loss = 0
            with open('trclassres.txt', 'w') as fout:
    ...
    for epoch in range(EPOCHS):
        ...
        if avg_vloss < best_vloss:
            ...
            bestflag = 1
            torch.save(model.state_dict(), 'molgl2hbeststate48-3400v2.pt')
        else:
            bestflag = 0
            torch.save(model.state_dict(), 'molgl2hlaststate48-3400v2.pt')
        epochidx += 1
        ...
        if bestflag == 1:
            ...
            torch.save(tinputlayer, INPLAYER)
            torch.save(tlastlayer, OUTLAYER)
            torch.save((classes, ttruelabs, tpredlabs), 'molgl2testlabs48-3400v2.pt')
